...
struct slab_cache_head _master;
struct slab_cache_head *slab_master = &_master;
...
void slab_init(void *heap_start, void *heap_end) {
  ...
  slab_master->cache_memory = (struct slab_cache *) (heap_start + slab_master->nr_pages * SZ_PAGE);
...
struct slab_cache *slab_cache_create(size_t size) {
  ...
  p_cache->pages_per_chain = (p_cache->items_per_chain * size + sizeof(struct slab_chain) - 1) / SZ_PAGE + 1;
  return p_cache;
}

void slab_cache_grow(struct slab_cache *p_cache) {
  void *page = NULL;
  struct slab_chain *p_chain;
