...
void disk_get_sections(struct Disk *disk) {
  ...
  Log("MBR    at %p, offset 0x%x", disk->mbr, (void *) disk->mbr - disk->head);

  disk->fsinfo = (struct FSInfo *) ((void *) disk->mbr + 512);
  Log("FSInfo at %p, offset 0x%x", disk->fsinfo, (void *) disk->fsinfo - disk->head);
  ...
  Log("FAT1   at %p, offset 0x%x", disk->fat[1], (void *) disk->fat[1] - disk->head);
  Log("FAT2   at %p, offset 0x%x", disk->fat[2], (void *) disk->fat[2] - disk->head);

  disk->fdt = (struct FDT **) (((void *) disk->fat[1]) + fatsz * disk->mbr->BPB_NumFATs);
  Log("DATA   at %p, offset 0x%x", disk->fdt, (void *) disk->fdt - disk->head);
}
